...
public class AddBookAction extends ActionSupport {
	...
	public String execute() 
	{
		try 
		{
			...
			{
				String pathStr = "book_images";

				String bookImageName = bookIsbn + choose_fileFileName.substring(choose_fileFileName.indexOf("."));
				//System.out.println(bookImageName);
				book.setBookImageAddress(pathStr + "/" + bookImageName); 
				if (administratorService.addBook(book,pathStr,bookImageName,this.choose_file))
				{
					return null;
				}
				return ERROR;
			...
			if (administratorService.addBook(book))
			{
				return null;
			}
			return ERROR;
