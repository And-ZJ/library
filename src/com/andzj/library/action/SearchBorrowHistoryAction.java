...
public class SearchBorrowHistoryAction extends ActionSupport{
	...
	public String execute() {
		try 
		{
			...
			{
				if (administratorService.findAllBorrowHistory())
				{
					return null;
				}
			}
			else if ("account".equals(searchMode))
			{
				...
				if (administratorService.searchBorrowHistoryByAccountName(accountName))
				{
					return null;
				}
			}
			else if ("isbn".equals(searchMode))
			{
				...
				if (administratorService.searchBorrowHistoryByBookIsbn(bookIsbn))
				{
					return null;
				}
			}
			...
			{
				String historyId = request.getParameter("search_words");
				if (administratorService.searchBorrowHistoryById(historyId))
				{
					return null;
				}
			}
